...
import { useState } from 'react';
import { Plus, Minus, Trash2, ChefHat, ShoppingBag } from 'lucide-react';
import { useCart, registerMenuItem } from '../context/CartContext';
...
export default function CompleteMenu() {
  ...
  return (
    <section className="py-12 md:py-20 bg-gradient-to-b from-black to-zinc-950" id="menu">
      <div className="container px-3 sm:px-4 md:px-6 lg:px-8 mx-auto">
        
        {/* Header - Responsive */}
        <div className="text-center mb-10 md:mb-16 px-2">
          <div className="inline-flex items-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-2 sm:py-3 mb-4 sm:mb-6 bg-gradient-to-r from-orange-500/10 to-red-500/10 rounded-full border border-orange-500/30">
            <ChefHat className="w-4 h-4 sm:w-5 sm:h-5 text-orange-400" />
            <span className="text-xs sm:text-sm md:text-base text-orange-400 font-semibold tracking-wide sm:tracking-wider">
              EXPLORE 155+ ITEMS
            </span>
          </div>
          <h2 className="mb-4 sm:mb-6 text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-bold text-white font-playfair">
            <span className="bg-gradient-to-r from-orange-400 via-yellow-400 to-orange-400 bg-clip-text text-transparent">
              Complete
            </span> Menu
          </h2>
          <p className="max-w-3xl mx-auto text-sm sm:text-base md:text-xl text-gray-400 px-2">
            From shakes to steaks - 155+ items crafted with passion
          </p>
        </div>
        ...
        <div className="mb-6 md:mb-8 text-center px-2">
          <div className="inline-flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-orange-500/10 to-red-500/10 rounded-xl">
            <ShoppingBag className="w-4 h-4 sm:w-5 sm:h-5 text-orange-400" />
            <span className="text-sm sm:text-base font-bold text-orange-400">Items in Cart: {cartCount}</span>
            {cartCount > 0 && (
              <button 
                onClick={() => {
                  const items = getCartItems();
                ...
                className="text-xs sm:text-sm text-orange-300 hover:text-orange-400 ml-2"
              >
                View Cart
              </button>
            )}
          ...
          <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
            {FILTERS.map((filter) => (
              <button
                key={filter.id}
                onClick={() => setActiveFilter(filter.id)}
                className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2.5 rounded-lg sm:rounded-xl transition-all duration-300 text-xs sm:text-sm ${
                  activeFilter === filter.id
                    ? 'bg-gradient-to-r from-orange-500 to-red-500 text-white shadow-lg sm:shadow-xl shadow-orange-500/30'
                    : 'bg-zinc-900 text-gray-400 hover:bg-zinc-800 hover:text-white'
                }`}
              >
        ...
        <div className="space-y-12 md:space-y-20">
          {getFilteredCategories().map((category) => (
            <div key={category.id} className="scroll-mt-20 px-2 sm:px-0">
              {/* Category Header */}
              <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-10">
                <div className="p-2 sm:p-3 bg-gradient-to-br from-orange-500/20 to-red-500/20 rounded-lg sm:rounded-xl">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                {category.items.map((item) => (
                  <div 
                    key={item.id}
                    className="group bg-gradient-to-b from-zinc-900/50 to-black rounded-xl sm:rounded-2xl overflow-hidden border border-zinc-800 hover:border-orange-500 transition-all duration-500 hover:shadow-xl hover:shadow-orange-500/10"
                  >
                    {/* Image */}
                    <div className="relative h-40 sm:h-48 overflow-hidden">
                      ...
                      <div className="absolute bottom-3 sm:bottom-4 right-3 sm:right-4">
                        <div className="px-3 sm:px-4 py-1.5 sm:py-2 bg-black/80 backdrop-blur-sm rounded-lg sm:rounded-xl">
                    ...
                    <div className="p-4 sm:p-6">
                      <h4 className="mb-2 text-base sm:text-lg md:text-xl font-bold text-white line-clamp-1">
                        {item.name}
                      </h4>
                      <p className="mb-4 sm:mb-6 text-xs sm:text-sm text-gray-400 line-clamp-2">
                        {item.description}
                      </p>

                      {/* Quantity & Add to Cart - Compact Design */}
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1.5 sm:gap-2">
                          <button 
                            onClick={() => updateQuantity(item.id, -1)}
                            className="flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-zinc-800 text-gray-300 hover:bg-zinc-700 transition-colors duration-200"
                          >
                            <Minus className="w-3 h-3 sm:w-4 sm:h-4" />
                          </button>
                          <span className="w-6 sm:w-8 text-center text-base sm:text-lg font-bold text-white">
                            {quantities[item.id] || 1}
                          </span>
                          <button 
                            onClick={() => updateQuantity(item.id, 1)}
                        ...
                        <div className="flex gap-1.5 sm:gap-2">
                          <button
                            onClick={() => handleRemoveItem(item.id)}
                            className="flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-colors duration-200"
                            title="Remove from cart"
                          >
                          ...
                          <button
                            id={`cart-btn-${item.id}`}
                            onClick={() => handleAddToCart(item)}
                            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-1.5 sm:py-2.5 text-xs sm:text-sm font-semibold text-white transition-all duration-300 bg-gradient-to-r from-orange-500 to-red-500 rounded-lg sm:rounded-xl hover:shadow-lg hover:shadow-orange-500/30 active:scale-95"
                          >
                            <span>Add</span>
