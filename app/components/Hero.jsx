...
export default function Hero() {
  ...
  const tr = (d, prop = 'opacity 0.8s ease, transform 0.8s ease') => ({
    opacity: ready ? 1 : 0,
  ...
  return (
    <section style={{ position:'relative', height:'100svh', minHeight:620, overflow:'hidden' }}>
      ...
      {/* Overlays */}
      <div style={{ position:'absolute', inset:0, background:'linear-gradient(to bottom, rgba(30,26,23,.55) 0%, rgba(30,26,23,.18) 45%, rgba(30,26,23,.68) 100%)', zIndex:1 }}/>
      <div style={{ position:'absolute', inset:0, background:'linear-gradient(to right, rgba(30,26,23,.25), transparent, rgba(30,26,23,.25))', zIndex:1 }}/>
      ...
      <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center', padding:'0 24px', zIndex:2 }}>
        ...
        <span style={{ ...tr('0.05s'), fontFamily:'var(--sans)', fontSize:'.62rem', fontWeight:600, letterSpacing:'.32em', textTransform:'uppercase', color:'rgba(243,237,226,.5)', marginBottom:28 }}>
          Okara · Punjab · Pakistan
        </span>

        <h1 style={{ ...tr('0.15s'), fontFamily:'var(--serif)', fontSize:'clamp(4rem,10vw,9.5rem)', fontWeight:400, lineHeight:.95, color:'#f3ede2', marginBottom:28 }}>
          <em style={{ fontStyle:'italic' }}>Cafe</em><br/>Janoshi
        </h1>

        <p style={{ ...tr('0.25s'), fontFamily:'var(--sans)', fontWeight:400, fontSize:'clamp(.9rem,1.8vw,1.1rem)', letterSpacing:'.06em', color:'rgba(243,237,226,.7)', lineHeight:1.75, maxWidth:480, marginBottom:48 }}>
          A refined dining experience in the heart of Okara —<br/>where every meal becomes a memory.
        </p>

        <div style={{ ...tr('0.35s'), display:'flex', gap:14, flexWrap:'wrap', justifyContent:'center' }}>
          <Link
            href="/menu"
            className="hero-btn-fill"
            style={{ display:'inline-flex', alignItems:'center', gap:9, fontFamily:'var(--sans)', fontSize:'.65rem', fontWeight:600, letterSpacing:'.22em', textTransform:'uppercase', padding:'14px 32px', background:'rgba(243,237,226,.95)', color:'var(--ink)', border:'1px solid rgba(243,237,226,.95)', transition:'all .45s var(--ease)', position:'relative', overflow:'hidden' }}
      ...
      {/* Scroll hint */}
      <div style={{ position:'absolute', bottom:32, left:'50%', transform:'translateX(-50%)', display:'flex', flexDirection:'column', alignItems:'center', gap:10, opacity: ready ? 1 : 0, transition:'opacity 0.8s ease 0.8s', zIndex:2 }}>
        <span style={{ fontFamily:'var(--sans)', fontSize:'.58rem', fontWeight:600, letterSpacing:'.26em', textTransform:'uppercase', color:'rgba(243,237,226,.38)' }}>Scroll</span>
        <div style={{ width:1, height:52, background:'linear-gradient(to bottom, rgba(243,237,226,.45), transparent)', animation:'hscroll 2.2s ease-in-out infinite' }}/>
      </div>
